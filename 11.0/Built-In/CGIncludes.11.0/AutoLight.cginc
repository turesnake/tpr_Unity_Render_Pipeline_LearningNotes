...
// ---- Screen space direction light shadows helpers (any version)
    ...
    #if defined(UNITY_NO_SCREENSPACE_SHADOWS) // 注 no screenspace cascaded shadowmaps
...
{
    //fade value
    // posWS 点积 WS->VS矩阵 的 第3行, 计算得到 poVS.z 值;
    float zDist = dot(_WorldSpaceCameraPos - worldPos, UNITY_MATRIX_V[2].xyz);
    float fadeDist = UnityComputeShadowFadeDistance(worldPos, zDist);
    half  realtimeToBakedShadowFade = UnityComputeShadowFade(fadeDist);// shadowFade
...
#   define UNITY_SHADOW_ATTENUATION(a, worldPos) SHADOW_ATTENUATION(a)
    ...
    // no lightmap uv thus store screenPos instead can happen if we have two directional lights.
    // main light gets handled in GI code, but 2nd dir light can have shadow screen and mask.
...
#       endif
#   else 
    // no shadowmask, 所以下面的 UnityComputeForwardShadows() 参数1 都为0, 将不执行对 shadowmask 的采样
...
#if !defined(UNITY_HALF_PRECISION_FRAGMENT_SHADER_REGISTERS)
    // 不支持 全尺寸浮点数精度 的平台
    #define DECLARE_LIGHT_COORD(input, worldPos) \ 
...
// _ShadowCoord.xyz: dirWS, 光源pos->目标点pos; (带模长)
...
#   define COMPUTE_LIGHT_COORDS(a) a._LightCoord = mul(unity_WorldToLight, mul(unity_ObjectToWorld, v.vertex)).xyz;
#   define LIGHT_ATTENUATION(a) \    
        (tex2D(_LightTextureB0, dot(a._LightCoord,a._LightCoord).rr).r * texCUBE(_LightTexture0, a._LightCoord).w * SHADOW_ATTENUATION(a))
#endif
