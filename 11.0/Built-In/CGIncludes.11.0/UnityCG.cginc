...
#if defined (DIRECTIONAL) || defined (DIRECTIONAL_COOKIE) || defined (POINT) || defined (SPOT) || 
    defined (POINT_NOATT) || defined (POINT_COOKIE)
#define USING_LIGHT_MULTI_COMPILE
#endif

#if defined(SHADER_API_D3D11) || defined(SHADER_API_PSSL) || defined(SHADER_API_METAL) || 
    defined(SHADER_API_GLCORE) || defined(SHADER_API_GLES3) || defined(SHADER_API_VULKAN) || 
    defined(SHADER_API_SWITCH) // D3D11, D3D12, XB1, PS4, iOS, macOS, tvOS, glcore, gles3, webgl2.0, Switch
...
inline half3 DecodeDirectionalLightmap  (half3 color, // 从  unity_Lightmap 采样得到的 单个值
                                        fixed4 dirTex, // 从 unity_LightmapInd 采样得到的 单个值
                                        half3 normalWorld // 法线, 包含 可法线贴图信息
){
    ...
    // 上面注释中提到的 specular lightmaps, 因为效果不好且消耗高, 已在 unity 5.6 后被废弃了;

    // 半兰伯特公式 = lightColor * diffuseColor * ( dot(Normal,LightDir) * 0.5 + 0.5)
    // 此处: dirTex.xyz 位于 [0,1], 减 0.5 后恰好变成 [-0.5, 0.5], 从而省略了 原始公式中 *0.5 这一步;
...
// ---
// 参数 z 是 z buffer, 若在 reverse-z 平台,其范围为[1,0]; 若在其它平台,其范围为[0,1]
// 此函数将 z 值映射到一个 空间中: 坐标系原点为0; 近平面为 n/f; 远平面为 1;
...
// ---
// 参数 z 是 z buffer, 若在 reverse-z 平台,其范围为[1,0]; 若在其它平台,其范围为[0,1]
// 此函数将 z 值映射到一个 空间中: 坐标系原点为0; 近平面为 n; 远平面为 f;
...
inline float4 ComputeNonStereoScreenPos(float4 pos) {
    // o.x = pos.x /2 + pos.w / 2;
    // o.y = pos.y /2 + pos.w / 2; (d3d平台, y要取反)
    // o.zw = pos.zw;
    // 因为是在 vertex shader 中执行的, 所以只能返回这个半成品
    float4 o = pos * 0.5f;
...
// 参数 normal: nomrlaOS
// ---
// 内部生成 posWS, 对其作用 normal bias, 再将其转换为 posCS
// 返回这个 posCS
float4 UnityClipSpaceShadowCasterPos(float4 vertex, float3 normal)
{
    ...
    if (unity_LightShadowBias.z != 0.0)
    {
        ...
        float3 wLight = normalize(UnityWorldSpaceLightDir(wPos.xyz));// 目标点pos->lightPos
...
// 参数 clipPos: 尚未"除w", 所以要考虑 w 的影响;
// 仅处理 平行光, spot光
float4 UnityApplyLinearShadowBias(float4 clipPos)
{
    // For point lights that support depth cube map, 
    // the bias is applied in the fragment shader sampling the shadow map.
    // This is because the legacy behaviour for point light shadow map cannot be implemented 
    // by offseting the vertex position in the vertex shader generating the shadow map.
    // ---
    // 对于那些支持 depth cube map 的 point光 来说, bias 将被用在 frag shader 中, 用来对 shadowmap 进行采样; 
    // 因为在 "生成 shadowmap 的 vertex shader" 中, 对顶点的坐标做偏移,是不适用的
#if !(defined(SHADOWS_CUBE) && defined(SHADOWS_CUBE_IN_DEPTH_TEX))
    #if defined(UNITY_REVERSED_Z)
        // We use max/min instead of clamp to ensure proper handling of the rare case
        // where both numerator and denominator are zero and the fraction becomes NaN.
        // 使用 max/min 来取代 clamp, 避免 分子分母都为零,进而导致 NaN 的极端情况;
        // ---
        // 因为 d3d 中, z值:[1->0], 值越小越远; 此处的 x值也是负的
        clipPos.z += max(-1, min(unity_LightShadowBias.x / clipPos.w, 0));// [-1,0]
    #else
        // 猜测 非d3d 平台中, z值:[-1,1],  x值可能是正的
        clipPos.z += saturate(unity_LightShadowBias.x/clipPos.w);
    #endif
#endif

// 平行光 使用的值, 支持针对 近平面做一下 clamp, 
#if defined(UNITY_REVERSED_Z)
...
//  UNITY_APPLY_FOG(fogData,col)            Applies fog to color "col". Automatically applies 
//                                          black fog when in forward-additive pass.
...
// 参数 coord 必须是 posCS.z, 未经 "除w" 的;
    ...
    #else
        // 似乎就没看见 定义了 UNITY_REVERSED_Z, 但其值又不是1 的源码...
        //GL with reversed z => z clip range is [near, -far] 
        //   -> should remap in theory but dont do it in practice to save some perf (range is close enough)
        // ---
        // 参数 coord 区间: [near, -far]
        // 只是做了反转
...
#else
    // OpenGL 主体平台:
    //Opengl => z clip range is [-near, far] 
    //     -> should remap in theory but dont do it in practice to save some perf (range is close enough)
    #define UNITY_Z_0_FAR_FROM_CLIPSPACE(coord) (coord)
#endif
...
// 参数 coord 是单个 float, 一般表示 距离值
//  有两种形式: -1- clip-space depth; 
//             -2- camera->fragment distance
#if defined(FOG_LINEAR)
    // factor = (end-z)/(end-start) = z * (-1/(end-start)) + (end/(end-start))
    // 返回值边界未受限, 需人为clamp为: [0,1], 仍表示衰减值, 1表示光线全通过, 无烟雾
