...
struct Unity_GlossyEnvironmentData
{
    ...
    half3   reflUVW; // 反射方向 (从 cubemap 中心射向 cube墙壁上的某一点)
};
...
// 这个分支不会执行:
    ...
    // 此分支得到的结果, 和上面 if 0 分支的 非常接近
    perceptualRoughness = perceptualRoughness*(1.7 - 0.7*perceptualRoughness);
#endif


    half mip = perceptualRoughnessToMipmapLevel(perceptualRoughness);//就是简单地乘以 mipmap 总级数
    half3 R = glossIn.reflUVW; // 就是 反射向量
