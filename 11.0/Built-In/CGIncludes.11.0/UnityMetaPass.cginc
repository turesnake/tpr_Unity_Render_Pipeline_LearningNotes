...
float4 UnityMetaVertexPosition( float4 vertex, 
    ...
    {
        // ---- 使用 uv1 ------ //
        // 等于是让 posOS 指向 lightmap texture 中, 本物体所在的那个 贴图区域的对应的 顶点pos
...
// 此函数会被执行两次, 一次用于 emission, 一次用于 diffuse
half4 UnityMetaFragment (UnityMetaInput IN)
{
    ...
    if (unity_MetaFragmentControl.x){
        ...
        res.rgb = clamp(
            pow (res.rgb, saturate(unity_OneOverOutputBoost) ), 
            0, 
