...
// ------------------------------------------------------------------
    ...
    // shadow sampling offsets and texel size
        ...
        float4 _ShadowOffsets[4]; // 四个采样点的 pos offset
...
inline fixed UnitySampleShadowmap (float4 shadowCoord)
{
        ...
        // 硬阴影
        // 1-tap shadows
        #if defined (SHADOWS_NATIVE) // gles2.0 以外的大部分平台都开启了此宏
            half shadow = UNITY_SAMPLE_SHADOW_PROJ(_ShadowMapTexture, shadowCoord);
            shadow = lerp(_LightShadowData.r, 1.0f, shadow);
        #else
            half shadow = SAMPLE_DEPTH_TEXTURE_PROJ(_ShadowMapTexture, UNITY_PROJ_COORD(shadowCoord)) < (shadowCoord.z / shadowCoord.w) ? 
...
half UnityMixRealtimeAndBakedShadows(   half realtimeShadowAttenuation, 
                                        half bakedShadowAttenuation, 
    ...
    // Pure realtime direct lit = N/A
            ...
            //In subtractive mode when there is no shadow 
            // we kill the light contribution as direct as been baked in the lightmap.
            return 0.0;
        #else
    ...
    //In distance shadowmask or realtime shadow fadeout we lerp toward the baked shadows 
    // (bakedShadowAttenuation will be 1 if no baked shadows)
...
// Shadow fade
// ------------------------------------------------------------------
// 得到 ShadowFadeDistance
float UnityComputeShadowFadeDistance(float3 wpos,   // frag posWS
                                    float z         // frag posVS.z
){
    float sphereDist = distance(wpos, unity_ShadowFadeCenterAndType.xyz);
    // 根据两种模式: Close Fit, Stable Fit, 二选一;
    return lerp(z, sphereDist, unity_ShadowFadeCenterAndType.w);
}

// ------------------------------------------------------------------
// 返回 阴影衰减值, [0,1], 0 表示无衰减, 1 表示全衰减,即 阴影没了
