...
// reflectivity:   直接反射光 占入射光总量 的比值
// 1-reflectivity: 剩余的光 的比值, 这些光, 有的被材质彻底吸收了, 有的再次反射出材质, 变成了漫反射光
//                 若忽略掉 彻底被材质吸收的光, 也可将这部分理解为 漫反射光 的比例;
...
// ret:
//   材质反照率 * 漫反射光的比例
inline half3 DiffuseAndSpecularFromMetallic (   half3 albedo, 
                                                half metallic, 
                                                out half3 specColor, 
                                                out half oneMinusReflectivity
){
    // unity_ColorSpaceDielectricSpec.rgb: 线性空间下为 0.04; 
...
// whiteout blending
// 整合 主法线贴图信息 n1, 和 detail法线贴图信息 n2,
// 得到 整合后的 法线贴图信息, 此值位于 切线空间中
...
//-------------------------------------------------------------------------------------
inline float3 BoxProjectedCubemapDirection (float3 worldRefl, // initial reflection direction
                                            float3 worldPos,  // 目标fragment posWS
                                            float4 cubemapCenter,  // w: 若值大于0, 表示开启了 box projection
                                            float4 boxMin,  // 包围盒 最左下后的那个端点 posWS
                                            float4 boxMax   // 包围盒 最右上前的那个端点 posWS
){
    // Do we have a valid reflection probe?
    // 因为 分支检测变量 cubemapCenter.w 是个 uniform 值 (针对所有像素点都是相同值)
	// 此时有必要开启 动态分支语句, 而不是任由编译器将这个 if...else... 展平
    UNITY_BRANCH
