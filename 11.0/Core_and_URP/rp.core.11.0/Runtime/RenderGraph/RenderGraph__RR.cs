...
namespace UnityEngine.Experimental.Rendering.RenderGraphModule
{
    ...
    [Flags]
    public enum DepthAccess//DepthAccess__
    {
        Read = 1 << 0,//1   
        Write = 1 << 1,//2
    ...
    public class RenderGraphContext//RenderGraphContext__
    {
        // context used for rendering
        public ScriptableRenderContext      renderContext;
        // Command Buffer used for rendering
        public CommandBuffer                cmd;
        // Render Graph pooll used for temporary data 临时数据
    ...
    /*
        初始化 RenderGraph 实例用的数据;
        本类仅被 "RenderGraph.Begin()" 使用;
    */
    public struct RenderGraphParameters//RenderGraphParameters__
    {
        ...
        // context used by the render pipeline
    ...
    */
    public class RenderGraph//RenderGraph__RR
    {
        ...
        // 存储本 render graph 容纳的所有 render passes
        ...
        /*
            使用 "AddRenderPass()" 中的参数 passName 生成 hash 值(int); 并用这个 passName 新建一个 ProfilingSampler 实例;
            这种 pair 数据缓存于此, 以供未来复用
        */
...
#region Public Interface  
        ...
        public string name { get; private set; } = "RenderGraph"; // Name of the Render Graph

        // If true, the Render Graph will generate execution debug information.
        internal static bool requireDebugData { get; set; } = false;

        // 在 pass 渲染代码 中可用的 默认资源集。
        public RenderGraphDefaultResources defaultResources
        {
            get
            {
        ...
        */
        ...
        /*
            ...
            Purge(清除) resources that have been used since last frame and resets internal states.

            这会释放 render graph 自上一帧以来未使用的任何资源。
            这也会在帧结束时, 执行 render graph 所需的所有内部处理。

            本函数 每一帧都要调用一次;
            必须在每一帧 最后一个 camera 完成渲染之后, 再调用;

            这是因为, 不同的 camera 可能执行不用的 rendering path( 比如 前向渲染 和 延迟渲染), 
            每个 camera 需要不同的资源;
        ...
        */
        ...
        /*
            ...
            ---
            如果一个 render pass 向一个 imported texture 写入数据, 这个 pass 就被认为存在副作用,
            它就不应该被 render graph 系统 cull 掉; 
            猜测:
                需要在这个 render pass 声明期间, 显示要求自己 不参与 cull
        */
        ...
        */
        ...
        /// <param name="sampler">Profiling sampler used around the pass.
        ///                         这个参数 通常是自动创建的, 见下方重载版
        /// </param>
        /// <returns>A new instance of a RenderGraphBuilder used to setup the new Render Pass.</returns>
        public RenderGraphBuilder AddRenderPass<PassData>(  string passName, 
        ...
        /*
            对 RenderGraph 实例的初始化;
            Begin using the render graph.
            此函数必须在 添加任何 render passes 之前调用;
        */
        ...
        */
        ...
        /*
            如果参数 name 对应的 sampler 已经存在, 返回之;
            若不存在, 创建一组新的;
        */
