...
namespace UnityEngine.Rendering
{
    ...
    /*
        ...
        为了能让 "带有 scale 值得 RTHandle texture" 能在 shader 中被采样,
        为了计算出正确得 uv 值, 提供了本类数据;
    */
    public struct RTHandleProperties//RTHandleProperties__
    {
        ...
        /*
            ...
            (x,y) current frame 
            (z,w) last frame 
            (this is only used for buffered RTHandle Systems)

            This is the scale to apply to full-screen UVs to sample an RTHandle.

            本类中最重要的数据;
            It allows you to scale "full-screen UV coords" 
            and use the result to sample an RTHandle. 比如:

                float2 scaledUVs = fullScreenUVs * rtHandleScale.xy;

            However, because the partial viewport always starts at (0, 0), when you use integer pixel coords 
            within the viewport to load content from a texture, there is no need to rescale them.

            Another important thing to consider is that, when you render a full-screen quad into a partial viewport, 
            there is no benefit from standard UV addressing mechanisms such as wrap or clamp. 
            This is because the texture might be bigger than the viewport. 
            For this reason, take care when you sample pixels outside of the viewport.
    ...
    /// </summary>
    public partial class RTHandleSystem//RTHandleSystem__RR
        : IDisposable
    {
        ...
        */
        ...
        /*
            ...
            提供最初的 "reference size";
            最好把 参数 width, height 直接设置成 full screen 尺寸, 以减少后面为所有 render texture 做重分配 的工作次数;

            如果需要 MSAA, 必须在 初始化时设置; RTHanlde 会为所有 texture 设置相同得采样数;
            你可以在后面改写 采样模式, 但是这个修改也是作用于所有 自动resized texture 的;
            (12.0 之后, 这个规则改了, 每个 texture 都能设置自己独立的采样配置,)
            
            在程序运行处理, 本函数只能被调用一次; 之后, 你就能使用这个被 初始化完毕的 实例, 去分配 texture 了;
        ...
        */
        ...
        public void Release(RTHandle rth)
        {
            if (rth != null)
            {
                Assert.AreEqual(this, rth.m_Owner);// 保证是同一个管理者
                rth.Release();
            }
        }
        ...
        /*
            ...
            --
            设置 "reference size"; 用于 default RTHandle System 的后续渲染;
            如果新的值大于 现有系统中记录的 "当前最大值"; 系统会对现存的所有 render texture 都执行扩容 (再分配)
            每个 texture 的容积为: current max "reference size" * 这个texture 的 scale;
        */
        ...
        */
        ...
        public void SetReferenceSize(int width, int height, MSAASamples msaaSamples, bool reset)
        {
...
#if UNITY_EDITOR
            // If the "reference size" is significantly higher than the current actualWidth/Height 
            // and it is larger than 1440p dimensions, we reset the "reference size" every several frames
            ...
            // If the currentViewportSize is 0, it mean we are the first frame of rendering 
            // (can happen when doing domain reload for example or for reflection probe)
            // in this case the scalePrevious below could be invalided. But some effect rely on having a correct value 
            // like TAA with the history buffer for the first frame.
        ...
        /*
            Returns the maximum allocated width/height of the RTHandle System.
            就是系统存储的 最大的 reference size;
        */
        ...
        */
        ...
        /// <param name="msaaSampleChanged"></param>
        ...
        */
        ...
        public RTHandle Alloc(
            ...
            // We need to handle this in an explicit way since GraphicsFormat does not expose depth formats. 
            // TODO: Get rid of this branch once GraphicsFormat'll expose depth related formats
            RenderTexture rt;
        ...
        /*
            ...
            RenderTextures allocated this way are meant to be(意味着) defined by a scale of camera resolution 
            (full/half/quarter resolution for example).
            The idea is that internally the system will scale up the size of all render texture 
            so that it amortizes(摊销) with time and not reallocate when a smaller size is required (which is what happens with TemporaryRTs).
            Since MSAA cannot be changed on the fly for a given RenderTexture, 
            a separate instance will be created if the user requires it. 
            This instance will be the one used after the next call of SetReferenceSize if MSAA is required.
        */
        ...
        /*
            Allocate a new "automatically" sized RTHandle. (1)
            ---
            由系统自动管理尺寸的;

            分辨率 = "max reference size" * scaleFactor;
        */
        /// <param name="scaleFactor">Constant scale for the RTHandle size computation. 
        ///                         就是 "scale" 值, (1,1) 表示 全屏, (0.5,0.5) 表示 1/4屏
        /// </param>
        /// <param name="slices">Number of slices of the RTHandle.</param>
        /// <param name="depthBufferBits">Bit depths of a depth buffer.</param>
        /// <param name="colorFormat">GraphicsFormat of a color buffer.</param>
        ...
        /// <returns></returns>
        public RTHandle Alloc(
            Vector2 scaleFactor, // 就是 "scale" 值
            int slices = 1,
            DepthBits depthBufferBits = DepthBits.None,
            GraphicsFormat colorFormat = GraphicsFormat.R8G8B8A8_SRGB,
            FilterMode filterMode = FilterMode.Point,
            ...
            // "max reference size" * scale; 且结果不能小于 1 像素;
        ...
        /*
            Allocate a new "automatically" sized RTHandle. (2)
            ---
            由系统自动管理尺寸的;
            本版本使用传入的函数 scaleFunc 来直接计算目标 render texture 的分辨率;
            此函数的参数是: max reference size (ve2)
            返回值是 目标 render texture 的像素分辨率 (Ve2);
        ...
        */
        /// <param name="scaleFunc">Function used for the RTHandle size computation.
        /// </param>
        /// <param name="slices">Number of slices of the RTHandle.</param>
        /// <param name="depthBufferBits">Bit depths of a depth buffer.</param>
        /// <param name="colorFormat">GraphicsFormat of a color buffer.</param>
