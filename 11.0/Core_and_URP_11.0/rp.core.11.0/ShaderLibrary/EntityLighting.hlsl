...
// Volume is store as 3D texture with 4 R, G, B, Occ set of 4 coefficient store atlas in same 3D texture. 
// Occ is use for occlusion.
// TODO: the packing here is inefficient as we will fetch values far away from each other 
//       and they may not fit into the cache - Suggest we pack RGB continuously
// TODO: The calcul of texcoord could be perform with a single matrix multicplication calcualted 
//       on C++ side that will fold probeVolumeMin and probeVolumeSizeInv into it 
//       and handle the identity case, no reasons to do it in C++ (ask Ionut about it)
...
void SampleProbeVolumeSH4(  TEXTURE3D_PARAM(SHVolumeTexture, SHVolumeSampler), 
                            float3 positionWS, 
                            float3 normalWS, 
                            float3 backNormalWS, 
                            float4x4 WorldToTexture,
                            float transformToLocal, // Computation are done in global space(0) or local space(1)
                            /*
                                Texel size on U texture coordinate 
                                当在 X(U) 轴上放置 N 个节点, 此值 = 1 / (4*N) .
                                    如, 当放置一个节点, 此值 = 0.25, 放置2个, 此值 = 0.125, 以此类推.
                                原因:
                                    3D texture 的单个元素是 float, 但我们需要为每个节点存储 3 个 float 值. 
                                    解决方案是把这个 3D texture 沿着 X轴 上再做 4等分, 每个位置存储一个 float, 
                                    4个一组就能存储一个节点需要的数据 (第四个元素会被弃用, 因为我们只需要存储 3 个 float)
                                    (这也是为什么 我们只需要 X(U) 轴的值, 而不需要 YZ 轴的, 因为 yz 轴没有被 4等分 )
                            */
                            float texelSizeX,       
                            float3 probeVolumeMin, 
                            float3 probeVolumeSizeInv,
                            inout float3 bakeDiffuseLighting, 
                            inout float3 backBakeDiffuseLighting)
{
    float3 position = (transformToLocal == 1.0) ? mul(WorldToTexture, float4(positionWS, 1.0)).xyz : positionWS;
    // 获得一个 xyz 三轴都在 [0,1] 区间内的 类似UV 的坐标值
...
float3 SampleProbeVolumeSH4(    TEXTURE3D_PARAM(SHVolumeTexture, SHVolumeSampler), 
                                float3 positionWS, 
                                float3 normalWS, 
                                float4x4 WorldToTexture,
                                float transformToLocal, 
                                float texelSizeX, 
                                float3 probeVolumeMin, 
                                float3 probeVolumeSizeInv)
{
...
// bakeDiffuseLighting and backBakeDiffuseLighting must be initialize outside the function
void SampleProbeVolumeSH9(  TEXTURE3D_PARAM(  SHVolumeTexture, SHVolumeSampler), 
                            float3 positionWS, 
                            float3 normalWS, 
                            float3 backNormalWS, 
                            float4x4 WorldToTexture,
...
// Just a shortcut that call function above
float3 SampleProbeVolumeSH9(    TEXTURE3D_PARAM(SHVolumeTexture, SHVolumeSampler), 
                                float3 positionWS, 
                                float3 normalWS, 
                                float4x4 WorldToTexture,
                                float transformToLocal, 
...
float4 SampleProbeOcclusion(    TEXTURE3D_PARAM(SHVolumeTexture, SHVolumeSampler), 
                                float3 positionWS, 
                                float4x4 WorldToTexture,
                                float transformToLocal, 
                                float texelSizeX, 
...
real3 SampleSingleLightmap( TEXTURE2D_LIGHTMAP_PARAM(lightmapTex, lightmapSampler), 
                            LIGHTMAP_EXTRA_ARGS, 
                            float4 transform,    // 就是 _ST 这种数据
                            bool encodedLightmap, // lightmap 是否被压缩
                            real4 decodeInstructions // 只有 xy 两个值被用到, 相关的 解码信息
){
...
void SampleDirectionalLightmap( TEXTURE2D_LIGHTMAP_PARAM(lightmapTex, lightmapSampler), 
