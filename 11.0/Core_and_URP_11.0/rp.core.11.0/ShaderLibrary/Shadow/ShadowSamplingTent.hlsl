...
/*
    传入你想要 采样的 texel size, atlas size, 以及采样点坐标: positionSTS
    此函数会根据 tent 滤波器规则, 帮你分配4个采样点, 返回给你这些采样点的 posUV 和 权重值
*/
// 3x3 Tent filter (45 degree sloped triangles in U and V)
void SampleShadow_ComputeSamples_Tent_3x3(  real4 shadowMapTexture_TexelSize, // xy: texelSize, zw: total textureSize
                                            real2 coord,                    // original sample position
                                            out real fetchesWeights[4], // weights of each samples
                                            out real2 fetchesUV[4])     // pos of each samples
{
