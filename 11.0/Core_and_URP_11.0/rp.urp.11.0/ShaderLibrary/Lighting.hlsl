...
struct BRDFData
{
    half3 diffuse;  // 漫反射 反射率 (分母是 "源自光源的总体入射光量", 因材质自身吸收了一部分光, 故需考虑 surface.albedo 的影响 )
    half3 specular; // 镜反   反射率 (分母是 "源自光源的总体入射光量", 因材质自身吸收了一部分光, 故需考虑 surface.albedo 的影响 )
    half reflectivity;
...
half OneMinusReflectivityMetallic(half metallic)
{
    ...
    half oneMinusDielectricSpec = kDielectricSpec.a; // 注意,这里取得是 alpha 值, 是 0.96
    return oneMinusDielectricSpec - metallic * oneMinusDielectricSpec;
}



inline void InitializeBRDFDataDirect(   half3 diffuse, 
                                        half3 specular, 
                                        half reflectivity, 
                                        half oneMinusReflectivity, 
                                        half smoothness, inout half alpha, 
...
inline void InitializeBRDFData( half3 albedo, 
                                half metallic, 
                                half3 specular, 
                                half smoothness, 
                                inout half alpha, 
...
#else
    // Matellic setup:  也是 CatLike 教程第三课: Directional Lights 中教授的模式
    half oneMinusReflectivity = OneMinusReflectivityMetallic(metallic);
    half reflectivity = 1.0 - oneMinusReflectivity;

    // ===== diffuse: =====
    // 在 [0, 0.96] 这和区间内, 用 (1-metallic) "非金属率" 去插值,
    // 获得的 比例值, 和 surface.color (实为反照率) 相乘, 获得 diffuse 反射率 
    // (分母是 "源自光源的总体入射光量", 因材质自身吸收了一部分光, 故需考虑 surface.color 的影响 )
    //
    // ===== specular: =====
    // 若沿用上面 diffuse 的逻辑, 就该用 metallic系数, 在区间 [0.04, 1] 之间做插值, 最后再乘以 surface.color
    // ---
    // 但这里存在一个现象:
    // -- 电介质的 镜反光是 无色相的(白色), 纯电介质时, 可直接设定为 (0.04, 0.04, 0.04).
    //    (而不是: 0.04 * surface.color, 这个值仍然有颜色倾向 )
    // -- 金属的 镜反光 通常带有色相.  纯金属时, 可以继续沿用: 100% * surface.color = surface.color;
    // 
    // (当 材质 和 光源都有颜色时, 光源的颜色会最先影响 镜反光, 当材质光滑度提高时, 材质自己的颜色就会盖过 光源的颜色)
    // ---
    // 使用简单的 线性插值来获得 specular 反射率 
    // (分母是 "源自光源的总体入射光量", 因材质自身吸收了一部分光, 故需考虑 surface.color 的影响 )
    //
    // 这样的计算存在一个问题: diffuse + specular 并不等于 surface.color...
...
// 针对这个函数的解释, 可在笔记中搜索: "函数_DirectBRDFSpecular.jpg"
                            ...
                            half3 viewDirectionWS)
{
    ...
    half LoH2 = LoH * LoH;
    // 注意,此处的 0.1h, 这个 "h" 后缀意为 half, 和 0.1f 相似  
    ...
    // ---
    // 在移动平台, half 类型不再等同于 float, 它的容量变小了, 此时公式的 分母部分有可能会溢出. 此处做了下 修正
...
// 注意, 这个函数名字 拼写 错了.... 然后它在另一个文件中, 用 宏改名 的方式 给兼容了....
// 但即便如此, 这个函数 已经被废弃了, 没有人会 调用到它
// tpr 在此手动将其注释掉
...
{
    /*float2 uv = UnityStereoTransformScreenSpaceTex(normalizedScreenSpaceUV);   tpr */
