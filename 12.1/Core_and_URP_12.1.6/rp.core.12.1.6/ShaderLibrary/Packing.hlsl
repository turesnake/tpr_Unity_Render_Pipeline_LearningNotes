...
// 参数n 的分量可以是任意值, 最终每个分量都被压缩到 [0,1] 区间;
...
// Tetrahedral(四面体) encoding - Looks like Tetra encoding 10:10 + 2 is similar to oct 11:11, as oct is cheaper prefer it
...
// Integer and Float packing not defined in GLES2
    ...
    {
        uint maxInt = (1u << numBits) - 1u;
        return (uint)(f * maxInt + 0.5); // Round(四舍五入) instead of truncating(截断)
    }
