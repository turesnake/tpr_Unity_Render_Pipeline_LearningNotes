...
// Safe for GLES2: HLSLcc will emulate(模仿) the missing operator ^, >> and rcp
float Hash(uint s)
{
    s = s ^ 2747636419u;
    s = s * 2654435769u;
    ...
    // Construct a float with half-open range [0, 1) using low 23 bits.   使用低23位构造半开范围[0,1)的浮点。
    // All zeros yields 0, all ones yields the next smallest representable value below 1.  全0表示0，全1表示小于1的次最小可表示值。
