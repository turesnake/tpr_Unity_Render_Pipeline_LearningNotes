...
// 本文件 仅被 RealtimeLights.hlsl 调用;

// Cookie: 
//      就是在 光源上额外增加的一个 遮罩层, 可以实现带纹样的 光照效果...
...
#include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/LightCookie/LightCookieInput.hlsl"
    ...
    // 似乎不会发生, 因为 本文件 仅被 RealtimeLights.hlsl 调用; 而在调用本文件之前, 已经先调用了 Shadows.hlsl, 里面强制定义了 REQUIRES_WORLD_SPACE_POS_INTERPOLATOR;
    // ---
    // 此处之所以这么写, 可能是因为, 万一本文件被某个 自定义 shader 单独调用, 可以保证此宏的有效性;
    // 但是 本文件内, 此宏也没有被使用...
