...
struct GrassVertexOutput
{
    ...
    // 若使用 lightmap,    声明: float2     lightmapUV  : TEXCOORD1;
...
void InitializeVertData(GrassVertexInput input, inout GrassVertexOutput vertData)
{
    ...
    // 若使用 lightmap, 写入: vertData.lightmapUV.xy = input.lightmapUV.xy * unity_LightmapST.xy + unity_LightmapST.zw;
    // 否则, 啥也不干;
    OUTPUT_LIGHTMAP_UV(input.lightmapUV, unity_LightmapST, vertData.lightmapUV);

    // 若使用 lightmap, 啥也不干;
    // 否则: 写入: vertData.vertexSH.xyz = SampleSHVertex(vertData.normal);
